...
void Tofsensor_In1_IRQHandler(void)
{
	...
	if(_flag==0)//先上升沿
	{
	  Tofsensor_Init_in1.Tofsensor_start_time=10000*(TIME_ISR_CNT+1)-(uint32_t)(Timer32_getValue(TIMER32_BASE+0x00020)/48);//单位us
		Tofsensor_In1_DN();
	  _flag=1;
	}
  else if(_flag==1)//后下降沿
	{		
	  Tofsensor_Init_in1.Tofsensor_end_time=10000*(TIME_ISR_CNT+1)-(uint32_t)(Timer32_getValue(TIMER32_BASE+0x00020)/48);//单位us
...
void Tofsensor_In2_IRQHandler(void)
{
	...
	if(_flag==0)//先上升沿
	{
	  ...
	  Tofsensor_Init_in2.Tofsensor_start_time=10000*(TIME_ISR_CNT+1)-(uint32_t)(Timer32_getValue(TIMER32_BASE+0x00020)/48);//单位us
		Tofsensor_In2_DN();
	  _flag=1;
	}
  else if(_flag==1)//后下降沿
	{		
	  Tofsensor_Init_in2.Tofsensor_end_time=10000*(TIME_ISR_CNT+1)-(uint32_t)(Timer32_getValue(TIMER32_BASE+0x00020)/48);//单位us
...
{	
	...
	{
		Tofsensor_Init_in2.pre_last_distance=Tofsensor_Init_in2.last_distance;//上上次高度
		Tofsensor_Init_in2.last_distance=Tofsensor_Init_in2.distance;//上次高度
		Tofsensor_Init_in2.distance=Tofsensor_Init_in2.Tofsensor_delta*3.0f;//本次高度		
    Tofsensor_Init_in2.last_vel=Tofsensor_Init_in2.vel;
		...
		{
			if(fault_flag==1&&Tofsensor_Init_in2.distance==Tofsensor_Init_in2.pre_last_distance)//本次=上上次
			{
				fault_flag=1;
			}			
			else if(fault_flag==1&&Tofsensor_Init_in2.distance==Tofsensor_Init_in2.last_distance)//本次=上次
			{
				fault_flag=1;
			}
			else if(fault_flag==1&&Tofsensor_Init_in2.last_distance==Tofsensor_Init_in2.pre_last_distance)//上次=上上次
			{
			  fault_flag=1;		
			}
			//以上为传感器异常处理
	...
	if(Tofsensor_Init_in2.Tofsensor_update_flag==1)//数据已解析，留作配用
	{
