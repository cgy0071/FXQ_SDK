...
void PPM_Init()
{
  MAP_GPIO_setAsInputPinWithPullDownResistor(GPIO_PORT_P6, GPIO_PIN3);
	GPIO_interruptEdgeSelect(GPIO_PORT_P6,GPIO_PIN3,GPIO_LOW_TO_HIGH_TRANSITION);
	MAP_GPIO_clearInterruptFlag(GPIO_PORT_P6, GPIO_PIN3);
	MAP_GPIO_enableInterrupt(GPIO_PORT_P6, GPIO_PIN3);
...
void PPM_IRQHandler(void)
{
	...
	now_ppm_time=10000*(TIME_ISR_CNT+1)-(uint32_t)(Timer32_getValue(TIMER32_BASE+0x00020)/48);//单位ms;//us
	//now_ppm_time=micros();
...
/***************************************************
函数名: void PORT1_IRQHandler(void)
说明:	PPM接收中断函数
入口:	无
出口:	无
备注:	程序初始化后、始终运行
