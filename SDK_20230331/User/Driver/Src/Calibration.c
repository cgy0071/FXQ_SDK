  ...
  if(Throttle_Control<=(1000+Buttom_Safe_Deadband)
     &&Yaw_Control>=Yaw_Max*Scale_Pecent_Max
       &&Roll_Control<=-Pit_Rol_Max*Scale_Pecent_Max
         &&Pitch_Control>=15)//Pit_Rol_Max*Scale_Pecent_Max)
  {
    Accel_Calibration_Makesure_Cnt++;
  }
  
  	//左摇杆朝右下偏一点点，右摇杆朝右下,持续3s
  if(((Throttle_Control<=(1000+Buttom_Safe_Deadband)
     &&Yaw_Control>=Yaw_Max*Scale_Pecent_Max
       &&Roll_Control<=-Pit_Rol_Max*Scale_Pecent_Max
         &&Pitch_Control>=15//Pit_Rol_Max*Scale_Pecent_Max
...
void Mag_Calibration_Check(void)
{
  ...
  //总共有三面需要校准，以下判断语句选择首次从哪一面校准，可选的为第一面或第二面
...
{
//Scale_Pecent_Max  0.75   //最大解锁幅值量程因子
//Pit_Rol_Max 30           //最大俯仰、横滚期望
//Yaw_Max     200          //最大偏航期望  
//Buttom_Safe_Deadband  100 //油门底部安全死区
  if(Throttle_Control<=(1000+Buttom_Safe_Deadband)
     &&Yaw_Control>=Yaw_Max*Scale_Pecent_Max  //150
       &&Roll_Control>=Pit_Rol_Max*Scale_Pecent_Max //22.5
         &&Pitch_Control<=-Pit_Rol_Max*Scale_Pecent_Max) //-22.5
    ESC_Calibration_Makesure_Cnt++;
  //else ESC_Calibration_Makesure_Cnt/=2;
  
  //左摇杆朝下偏右一点点，右摇杆朝左下持续5s
  if(Throttle_Control<=(1000+Buttom_Safe_Deadband)
     &&Yaw_Control>=Yaw_Max*Scale_Pecent_Max
       &&Roll_Control>=Pit_Rol_Max*Scale_Pecent_Max
