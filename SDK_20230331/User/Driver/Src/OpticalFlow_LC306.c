...
#define OPTICALFLOW_UART EUSCI_A0_BASE
...
uint8_t Config_Init_Uart()
{
  ...
  {
    while((MAP_UART_getEnabledInterruptStatus(OPTICALFLOW_UART)&EUSCI_A_UART_RECEIVE_INTERRUPT_FLAG)==FALSE)
    {
      LC306_Ready_Cnt--;
      if(LC306_Ready_Cnt==0) return 0;
    };
    recv[recv_cnt++] = MAP_UART_receiveData(OPTICALFLOW_UART);							
  }			
