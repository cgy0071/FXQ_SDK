...
void TFmini_Statemachine(void)
{
  ...
  if(tfmini_cnt>=3)//20ms=50hz
  {
		...
		{
			if(COM1_Rx_Buf.Ring_Buff[i]==0x59
				&&COM1_Rx_Buf.Ring_Buff[i+1]==0x59)
			{
			  tfdata.distance=(COM1_Rx_Buf.Ring_Buff[i+3]*256)+COM1_Rx_Buf.Ring_Buff[i+2];
				tfdata.strength=(COM1_Rx_Buf.Ring_Buff[i+4]*256)+COM1_Rx_Buf.Ring_Buff[i+5];
				tfdata.temperature=((COM1_Rx_Buf.Ring_Buff[i+6]*256)+COM1_Rx_Buf.Ring_Buff[i+7])/8.0f-256;
