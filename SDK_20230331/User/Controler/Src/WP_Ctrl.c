...
//			{
				...
				{
				  Total_Controller.Yaw_Angle_Control.Expect=Yaw;//这里差距了11°，不知道为什么达不到期望，总是差11左右度
				}
				Total_Controller.Yaw_Angle_Control.FeedBack=Yaw;//-11;//偏航角反馈
...
{	
  ...
  //偏航角控制
  Total_Controller.Yaw_Gyro_Control.FeedBack=Yaw_Gyro;//Yaw_Gyro_Earth_Frame
...
/************************第五通道控制高度模式************************/		
	...
	{
		Throttle=Throttle_Control;//油门直接来源于遥控器油门给定//pid//Throttle_Control
...
/************************第八通道控制水平位置模式************************/		
			...
			Total_Controller.High_Position_Control.Expect=150;   //********定高50
			Flight_Alt_Hold_Control(ALTHOLD_MANUAL_CTRL,NUL,NUL);//高度控制		 NUL//ALTHOLD_AUTO_VEL_CTRL//ALTHOLD_MANUAL_CTRL
...
备注:	上电初始化，运行一次
****************************************************/
int time_cnt_jiangluo=0;
int flag_jiangluo=0;
void Total_Control(void)
{
	// 返航模式2，非返航1
    Controler_Land_Mode        =  1;//*******//
	// 水平自稳2，非自稳1
	Controler_Horizontal_Mode  =  2;//*********//光流2，遥控1
	// 高度自控2，非高度1
  ...
  if (PPM_Databuf[4] > 1500) {
	  ...
	  if (1/*start_cnt >= 1000*/) {
		...
		/*
		OpticalFlow_Control_Pure(0);
		if(ABS(100-NamelessQuad.Position[_YAW])<=2.0f)
			Flight_Alt_Hold_Control(ALTHOLD_AUTO_VEL_CTRL, 100, 0);
		else
			Flight_Alt_Hold_Control(ALTHOLD_AUTO_VEL_CTRL, 0, 10);
		*/
		  Main_Leading_Control();	
		//Auto_Flight_Ctrl(1/*SDK1_Mode_Setup*/);
	  }
  }
