...
float Temperature_Setpoint=50;
const float Temp_Control_Unit[20]=
{1  ,1 ,1 ,0 ,0 ,0 , 0 ,50    ,30  ,0 ,80,  8.00  ,0.75   ,125.0  ,0    ,0 ,100 , 1 ,  1 ,  1 };//水平维度方向加速度控制器 20 3 50  //  15  1  150
/*
...
void Simulation_PWM_Output(uint16_t width)
{
  ...
  if(cnt<=width) MAP_GPIO_setOutputHighOnPin(GPIO_PORT_P9,GPIO_PIN1);
	else MAP_GPIO_setOutputLowOnPin(GPIO_PORT_P9,GPIO_PIN1);
#else
	MAP_GPIO_setOutputLowOnPin(GPIO_PORT_P9,GPIO_PIN1);
#endif
...
{
#if Temperature_Ctrl_Enable
  return (ABS(Total_Controller.IMU_Temperature_Control.Err))<=1.0f?1:0;
#else
	return 1;
#endif	
}
