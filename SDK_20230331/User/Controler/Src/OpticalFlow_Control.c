...
Vector2f OpticalFlow_Pos_Ctrl_Last_Err={0};
...
void OpticalFlow_Pos_Control(void)
{
  ...
  OpticalFlow_Pos_Ctrl_Cnt++;
	...
	OpticalFlow_Pos_Ctrl_Last_Err.x=OpticalFlow_Pos_Ctrl_Err.x;
	OpticalFlow_Pos_Ctrl_Last_Err.y=OpticalFlow_Pos_Ctrl_Err.y;
  ...
  if(OpticalFlow_Pos_Ctrl_Cnt>=4)//10ms控制一次速度，避免输入频率过大，系统响应不过来
  {
    ...
    OpticalFlow_Pos_Ctrl_Err.y=constrain_float(OpticalFlow_Pos_Ctrl_Err.y,-Total_Controller.Optical_Position_Control.Err_Max,Total_Controller.Optical_Position_Control.Err_Max);		
		...
		OpticalFlow_Pos_Ctrl_Last_Err.x=constrain_float(OpticalFlow_Pos_Ctrl_Last_Err.x,-Total_Controller.Optical_Position_Control.Err_Max,Total_Controller.Optical_Position_Control.Err_Max);//100
    OpticalFlow_Pos_Ctrl_Last_Err.y=constrain_float(OpticalFlow_Pos_Ctrl_Last_Err.y,-Total_Controller.Optical_Position_Control.Err_Max,Total_Controller.Optical_Position_Control.Err_Max);	
		
		//Controler->Kd*(Controler->Err-Controler->Last_Err)
		
    //计算位置控制输出
    OpticalFlow_Pos_Ctrl_Output.x=Total_Controller.Optical_Position_Control.Kp*OpticalFlow_Pos_Ctrl_Err.x+Total_Controller.Optical_Position_Control.Kd*(OpticalFlow_Pos_Ctrl_Err.x-OpticalFlow_Pos_Ctrl_Last_Err.x);
    OpticalFlow_Pos_Ctrl_Output.y=Total_Controller.Optical_Position_Control.Kp*OpticalFlow_Pos_Ctrl_Err.y+Total_Controller.Optical_Position_Control.Kd*(OpticalFlow_Pos_Ctrl_Err.y-OpticalFlow_Pos_Ctrl_Last_Err.y);
	...
	{	
	  ...
	  if(Opv_Top_View_Target.target_ctrl_enable==1)//目标点检测跟踪
		{			
			...
			Total_Controller.SDK_Roll_Position_Control.Expect=5.1;//0;
			...
			Total_Controller.SDK_Pitch_Position_Control.Expect=8;//0;
...
//			PID_Control_SDK_Err_LPF(&Total_Controller.SDK_Pitch_Position_Control,Opv_Top_View_Target.trust_flag);
	...
	{
		OpticalFlow_Vel_Control(OpticalFlow_Pos_Ctrl_Output);//速度控制周期20ms
		Controller.yaw_outer_control_output  =-3.0f*Opv_Top_View_Target.sdk_angle;//偏航用角度控制
	}			
}
