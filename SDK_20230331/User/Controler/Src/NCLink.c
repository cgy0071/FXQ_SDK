...
/***************************************************************************************
@函数名：Serial_Data_Send_Dma(uint8_t *buf, uint32_t cnt)
@入口参数：buf:待发送数据
			     cnt:待发送字长
@出口参数：无
功能描述：串口发送函数
...
@日期：2020年01月17日
****************************************************************************************/
void Serial_Data_Send(uint32_t cnt)
{
#if Use_Dma_Enable
	Serial_Data_Send_Dma((u32)(&nclink_databuf),cnt);
#else	
	USART2_Send(nclink_databuf,cnt);
#endif
}
...
{
	Bling_Set(&Light_3,500,50,0.1,0,GPIO_PORT_PJ,GPIO_PIN5,0);//红色  //用户移植时，重写此函数
}
...
{
  uint8_t sum = 0,i=0,_cnt=0;
  nclink_databuf[_cnt++]=NCLink_Head[0];
  nclink_databuf[_cnt++]=NCLink_Head[1];
  nclink_databuf[_cnt++]=NCLINK_SEND_CHECK;
  nclink_databuf[_cnt++]=1;
  nclink_databuf[_cnt++]=response;
...
{
	static uint8_t nclink_rec_buf[1024];
...
void NCLink_Data_Prase_Process(uint8_t *data_buf,uint8_t num)//飞控数据解析进程
{
  ...
  else if(*(data_buf+2)==0X09)                             //遥控器参数
  {
    ...
    PPM_Databuf[7]=(int16_t)(*(data_buf+18)<<8)|*(data_buf+19);
		PPM_Databuf[8]=(int16_t)(*(data_buf+20)<<8)|*(data_buf+21);
    PPM_Databuf[9]=(int16_t)(*(data_buf+22)<<8)|*(data_buf+23);
  ...
  {		
    ...
    ngs_sdk.update_flag=true;
		...
		switch(*(data_buf+4))
		{
			case SDK_FRONT: ngs_sdk.move_flag.sdk_front_flag=true;  ngs_sdk.f_distance= ngs_sdk.move_distance;	break;
			case SDK_BEHIND:ngs_sdk.move_flag.sdk_behind_flag=true; ngs_sdk.f_distance=-ngs_sdk.move_distance;	break;
			case SDK_LEFT:  ngs_sdk.move_flag.sdk_left_flag=true;	  ngs_sdk.f_distance= ngs_sdk.move_distance;	break;
			case SDK_RIGHT: ngs_sdk.move_flag.sdk_right_flag=true;  ngs_sdk.f_distance=-ngs_sdk.move_distance;	break;
			case SDK_UP:    ngs_sdk.move_flag.sdk_up_flag=true;     ngs_sdk.f_distance= ngs_sdk.move_distance;	break;
			case SDK_DOWN:  ngs_sdk.move_flag.sdk_down_flag=true;   ngs_sdk.f_distance=-ngs_sdk.move_distance;	break;			
		}
	...
	if(!NCLink_Send_Check_Status_Parameter())//判断地面站有无请求数据、是否需要发送应答
	{
		if(NCLink_Cnt==0)//飞控姿态等基本信息
		{
			...
			NCLink_Send_Status(Roll,Pitch,Yaw,
												 ...
												 WP_Sensor.temperature,Battery_Voltage,Controler_High_Mode,Controler_State);
		}
...
//			NCLink_Send_GPSData(Longitude_Origion,Latitude_Origion,Altitude_Origion,GPS_Pos_DOP,GPS_FixType,GPS_Sate_Num);
		...
		{
			NCLink_Cnt=5;
			//NCLink_Send_Obs_NE(WP_Sensor.gyro_raw.x,WP_Sensor.gyro_raw.y,WP_Sensor.gyro_raw.z,0);
		}
		else if(NCLink_Cnt==5)//飞控竖直观测位置、光流速度
		{
			NCLink_Cnt=6;
			NCLink_Send_Obs_UOP(WP_Sensor.baro_altitude,Ground_Distance,opt_gyro_data.y,opt_gyro_data.x);
		}
		...
		{
			NCLink_Cnt=9;
			NCLink_Send_Userdata(Roll,Pitch,Yaw,
													 Roll_Observation,Pitch_Observation,Yaw_Observation);
		}
		else if(NCLink_Cnt==9)//传感器校准原始数据
		{
			NCLink_Cnt=10;
		  NCLink_Send_CalRawdata1(Gyro_Safety_Calibration_Flag,
			                       WP_Sensor.gyro_raw.x*GYRO_CALIBRATION_COFF,
														 WP_Sensor.gyro_raw.y*GYRO_CALIBRATION_COFF,
														 WP_Sensor.gyro_raw.z*GYRO_CALIBRATION_COFF,
