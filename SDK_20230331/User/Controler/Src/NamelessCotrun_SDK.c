...
extern uint8_t find_black(uint16_t transition_time,SDK_Status *Status,uint16_t number);
...
uint8_t move_with_speed_target(float x_target,float y_target,float delta,SDK_Status *Status,uint16_t number)
{
  ...
  else
  {
    ...
    if(dt.Now_Time>end_time)
    {
      ...
      end_time=0;
      Status->Transition_Time[number]=2000;//1000*5ms=5s
      return 1;//返回完成
    }
    else
    { 
...
uint8_t move_with_xy_target(float pos_x_target,float pos_y_target,SDK_Status *Status,uint16_t number)
{
  ...
  {    
    if(pythagorous2(OpticalFlow_Pos_Ctrl_Expect.x-OpticalFlow_SINS.Position[_PITCH],
                    OpticalFlow_Pos_Ctrl_Expect.y-OpticalFlow_SINS.Position[_ROLL])<=5.0f)
    {
      Status->Status[number].Execute_Flag=1;
      Status->Status[number].End_flag=1;
      //pos_base.x=0;
  ...
  {   		//开始后只配置一次
    ...
    else if(end_flag==2)
    {
      ...
      if(ABS(target_alt-NamelessQuad.Position[_YAW])<=5.0f)//结束条件
      {
        ...
        Status->Status[number].Execute_Flag=1;
        Status->Status[number].End_flag=1;
        Status->Transition_Time[number]=200;//1000*5ms
...
#define NCQ_SDK_DUTY1 move_with_z_target(150,10,0,&SDK_Duty_Status,1-1)
#define NCQ_SDK_DUTY2 move_with_xy_target( -50, 125,&SDK_Duty_Status,2-1)		///*相对距坐标*/ 125 200
#define NCQ_SDK_DUTY3 move_with_xy_target( -225, -75,&SDK_Duty_Status,3-1)  //find_black(200,&SDK_Duty_Status, 3-1) 150 150
#define NCQ_SDK_DUTY4 move_with_xy_target( 275, -50,&SDK_Duty_Status,4-1)//find_black(200,&SDK_Duty_Status, 3-1)
#define NCQ_SDK_DUTY5 find_black(200,&SDK_Duty_Status, 5-1)
#define NCQ_SDK_DUTY6 move_with_z_target(0,-5,0,&SDK_Duty_Status,6-1)
...
#define NCQ_SDK_DUTY6 move_with_speed_target(0, 30, 5000, &SDK_Duty_Status, 6-1)
...
void NCQ_SDK_Run(void)
{
  ...
  else if(SDK_Duty_Cnt==1)   {NCQ_SDK_DUTY2; SDK_Ground_updata=1; Ground_OPENMV_mode=2;}
  else if(SDK_Duty_Cnt==2)   {NCQ_SDK_DUTY3; SDK_Ground_updata=1; Ground_OPENMV_mode=1;}
...
void SDK_Data_Prase_1(void)
{
  ...
  {
    if(COM3_Rx_Buf.Tail<SDK_Target_Length)//0-36数据位正在传输
    {
