...
void US200_IRQHandler(void)
{
	...
	if(_flag==0)//先上升沿
	{
	  us200.us200_start_time=10000*(TIME_ISR_CNT+1)-(uint32_t)(Timer32_getValue(TIMER32_BASE+0x00020)/48);//单位us
		US200_DN();
	  _flag=1;
	}
  else if(_flag==1)//后下降沿
	{
	  us200.us200_end_time=10000*(TIME_ISR_CNT+1)-(uint32_t)(Timer32_getValue(TIMER32_BASE+0x00020)/48);//单位us
...
void US_200_Statemachine(void)
{
	...
	{
		us200.pre_last_distance=us200.last_distance;//上上次高度
		us200.last_distance=us200.distance;//上次高度
		us200.distance=us200.us200_delta*(340)/20000.0f;//本次高度		
		...
		{
			if(fault_flag==1&&us200.distance==us200.pre_last_distance)//本次=上上次
			{
				fault_flag=1;
			}			
			else if(fault_flag==1&&us200.distance==us200.last_distance)//本次=上次
			{
				fault_flag=1;
			}
			else if(fault_flag==1&&us200.last_distance==us200.pre_last_distance)//上次=上上次
			{
			  fault_flag=1;		
			}
			//以上为传感器异常处理
  ...
  if(Front_Distance<=500&&Front_Distance>0)  Sensor_Flag.Front_Health=1;//250原来是500
